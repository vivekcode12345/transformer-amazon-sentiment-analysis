"""Central configuration for text-only and multimodal AI pipelines.
...
- Multimodal image+text fusion (future Health AI applications)
...
# ============================================================================
# MULTIMODAL CONFIGURATION (Image + Text Fusion)
# ============================================================================

@dataclass(frozen=True)
class MultimodalTrainingConfig:
    """Hyperparameters for multimodal (image + text) training.
    
    This is a stub for future phases. Will be filled as image encoder
    and fusion layer are implemented.
    """

    # Image encoder config (stub)
    image_encoder_name: str = "clip"  # Placeholder: "clip", "blip", "siglip", etc.
    image_encoder_hidden_dim: int = 512

    # Text encoder config (reuses BERT)
    text_encoder_name: str = "bert-base-uncased"
    text_encoder_hidden_dim: int = 768

    # Fusion strategy config (stub)
    fusion_strategy: str = "concat"  # "concat", "attention", "cross_modal", etc.
...
class PathConfig:
    ...
    # Multimodal pipeline artifacts (future)
    multimodal_output_dir: Path = models_dir / "multimodal_finetuned"
    multimodal_logging_dir: Path = logs_dir / "multimodal_trainer_logs"
    multimodal_best_model_dir: Path = multimodal_output_dir / "best_model"
    multimodal_checkpoints_dir: Path = multimodal_output_dir / "checkpoints"
    multimodal_train_metrics_path: Path = reports_dir / "multimodal_train_metrics.json"
    multimodal_eval_metrics_path: Path = reports_dir / "multimodal_eval_metrics.json"
...
class RuntimeConfig:
    ...
    # Pipeline selection
    # Set enable_multimodal=True to run multimodal pipeline (requires image data)
...
def create_project_dirs() -> None:
    ...
    multimodal_directories = [
        PATH_CONFIG.multimodal_output_dir,
        PATH_CONFIG.multimodal_logging_dir,
        PATH_CONFIG.multimodal_best_model_dir,
        PATH_CONFIG.multimodal_checkpoints_dir,
    ]

    for directory in text_directories + multimodal_directories:
