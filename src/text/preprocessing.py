"""Preprocessing utilities for text-only sentiment analysis."""
...
def stream_tokenized_train(
    ...
    The returned object is an IterableDataset that tokenizes batches on-the-fly.
    Does NOT materialize the entire training split into memory.
    """
...
def materialize_validation_from_test(
    ...
    resolved_size = (
        validation_size if validation_size is not None else TEXT_CONFIG.validation_materialize_size
    )
...
def prepare_tokenized_datasets(
    ...
    resolved_seed = TEXT_CONFIG.random_seed if random_seed is None else random_seed
