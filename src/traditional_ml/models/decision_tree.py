...
def train_decision_tree(
    ...
    random_state: int = 42,
    clean: bool = True,
) -> Tuple[DecisionTreeClassifier, Any, Any, Any, Any, Dict[str, Any]]:
    """
    ...
    Example:
        >>> model, X_train, X_test, y_train, y_test, metrics = train_decision_tree()
        >>> print(f"Accuracy: {metrics['accuracy']:.2%}")
    """
    
    print("\n" + "=" * 70)
    print("DECISION TREE - TRAINING PIPELINE")
    print("=" * 70)
    ...
    # =========================================================================
    print("\n[Step 2/6] Initializing Decision Tree model...")
    print(f"   Configuration: {DECISION_TREE_CONFIG}")
    
    # Create the model with our hyperparameters
    model = DecisionTreeClassifier(**DECISION_TREE_CONFIG)
    
    print(f"   Model: {model.__class__.__name__}")
    print(f"   Criterion: {DECISION_TREE_CONFIG['criterion']}")
    print(f"   Max depth: {DECISION_TREE_CONFIG['max_depth']}")
    print(f"   Min samples split: {DECISION_TREE_CONFIG['min_samples_split']}")
    print(f"   Min samples leaf: {DECISION_TREE_CONFIG['min_samples_leaf']}")
    ...
    # The model learns a tree structure to separate positive and negative reviews
    ...
    metrics = evaluate_decision_tree(y_test, y_pred, y_pred_proba)
    ...
    print("=" * 70)
    print("\nYou can now:")
    print("1. Load the model: model = joblib.load('models/traditional_ml/decision_tree/model.pkl')")
    print("2. Make predictions: model.predict(new_tfidf_features)")
    print("3. View metrics: reports/decision_tree_metrics.json")
    print("=" * 70)
