...
def train_random_forest(
    ...
    random_state: int = 42,
    clean: bool = True,
) -> Tuple[RandomForestClassifier, Any, Any, Any, Any, Dict[str, Any]]:
    """
    ...
    Example:
        >>> model, X_train, X_test, y_train, y_test, metrics = train_random_forest()
    ...
    print(f"   Configuration: {RANDOM_FOREST_CONFIG}")
    
    # Create the model with our hyperparameters
    model = RandomForestClassifier(**RANDOM_FOREST_CONFIG)
    
    print(f"   Model: {model.__class__.__name__}")
    print(f"   Number of estimators: {RANDOM_FOREST_CONFIG['n_estimators']}")
    print(f"   Criterion: {RANDOM_FOREST_CONFIG['criterion']}")
    print(f"   Max depth: {RANDOM_FOREST_CONFIG['max_depth']}")
    print(f"   Class weight: {RANDOM_FOREST_CONFIG['class_weight']}")
    ...
    # The model builds multiple decision trees and combines their predictions
    ...
    metrics = evaluate_random_forest(y_test, y_pred, y_pred_proba)
    ...
    print("=" * 70)
    print("\nYou can now:")
    print("1. Load the model: model = joblib.load('models/traditional_ml/random_forest/model.pkl')")
    print("2. Make predictions: model.predict(new_tfidf_features)")
    print("3. View metrics: reports/random_forest_metrics.json")
    print("=" * 70)
